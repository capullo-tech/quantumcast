package com.quantumcast.viewmodel
...
import com.quantumcast.data.db.AppDatabase
import com.quantumcast.data.db.FavoriteEntity
import com.quantumcast.data.db.FavoriteGroupEntity
...
import com.quantumcast.data.repository.RadioRepository
import com.quantumcast.data.settings.AppSettings
import com.quantumcast.data.settings.SettingsRepository
import com.quantumcast.player.PlaybackService
import com.quantumcast.shazam.AudioCapturer
import com.quantumcast.shazam.ShazamRecognizer
...
data class PlayerState(
    ...
    val icyTitle: String = "",
    val currentTrack: com.quantumcast.data.model.TrackLookup? = null,
)
...
class RadioViewModel(app: Application) : AndroidViewModel(app) {
    ...
    val snapclientState: StateFlow<com.quantumcast.snapcast.SnapclientProcess.ConnectionState> = _snapclientState

    private val _snapcastGroups = MutableStateFlow<List<com.quantumcast.snapcast.Group>>(emptyList())
    val snapcastGroups: StateFlow<List<com.quantumcast.snapcast.Group>> = _snapcastGroups
            ...
            serviceStateJob = viewModelScope.launch {
                svc.state.collect { svcState ->
                    _playerState.update { ps ->
                        val isSnapclient = svcState.broadcastMode == com.quantumcast.data.settings.BroadcastMode.SNAPCLIENT
                        val updatedStation = if (isSnapclient) {
                            ...
                            if (displayName.isNotBlank() || svcState.snapcastCountry.isNotBlank()) {
                                val base = ps.station ?: com.quantumcast.data.model.Station()
                                base.copy(
                        ...
                        val snapTrack = if (isSnapclient && svcState.snapcastArtistName.isNotBlank()) {
                            com.quantumcast.data.model.TrackLookup(
                    ...
                    _isStreamLocked.value = if (svcState.broadcastMode == com.quantumcast.data.settings.BroadcastMode.SNAPCLIENT) {
    ...
    fun connectToSnapserver(host: String, port: Int = 1604) {
        ...
        _playerState.update { it.copy(
            station = com.quantumcast.data.model.Station(uuid = "snapclient-$host", name = host),
    ...
    fun setShareService(v: com.quantumcast.data.settings.ShareService) { viewModelScope.launch { settingsRepo.setShareService(v) } }
    ...
    // --- ADB self-test ---
    // Trigger: adb shell am start -n com.quantumcast/.MainActivity --es dbg snaptest
    ...
    fun startSnapTest() {
        ...
        playbackService?.playStation(
            ...
            uuid     = "snaptest-001",
            favicon  = "",
            broadcastMode = com.quantumcast.data.settings.BroadcastMode.QUANTUMCAST,
