...
import com.quantumcast.viewmodel.UiState
...
fun SearchScreen(
    ...
    fun cycleSort(clicked: SortBy) {
        when {
            sortBy != clicked -> { sortBy = clicked; sortDir = SortDir.DESC }
            ...
            OutlinedTextField(
                ...
                trailingIcon = {
                    if (fieldText.isNotEmpty() || phase == SearchPhase.FILTER) {
                        IconButton(onClick = {
                            if (phase == SearchPhase.FILTER) resetToSearch()
                            else fieldText = ""
                        }) {
                            Icon(
                            ...
                            DropdownMenu(expanded = showSortMenu, onDismissRequest = { showSortMenu = false }) {
                                listOf(SortBy.KBPS to "kbps", SortBy.VOTES to "Votes", SortBy.CLICKS to "Clicks")
                                    .forEach { (sort, label) ->
                                        DropdownMenuItem(
                                            text = { Text(label) },
                                            onClick = { cycleSort(sort); showSortMenu = false },
                                ...
                                if (sortBy != null) {
                                    ...
                                    DropdownMenuItem(
                                        text = { Text("Clear") },
                                        onClick = { sortBy = null; saveSortToVm(); showSortMenu = false }
                                    )
                                }
                            }
                        }
...
fun QuantumEntangleIcon(
    ...
    val centerAlpha by animateFloatAsState(
        ...
        animationSpec = tween(350), label = "qcenter"
    )
    Canvas(modifier) {
